...
bool parseBitcoinURI(const QUrl &uri, SendCoinsRecipient *out)
{
    ...
    if(uri.scheme() != QString("lanacoin"))
        return false;
...
{
    // Convert lanacoin:// to lanacoin:
    //
    //    Cannot handle this later, because bitcoin:// will cause Qt to see the part after // as host,
    //    which will lower-case it (and thus invalidate the address).
    if(uri.startsWith("lanacoin://"))
    {
        uri.replace(0, 12, "lanacoin:");
    }
...
{
    return GetSpecialFolderPath(CSIDL_STARTUP) / "Lanacoin.lnk";
}
...
{
    return GetAutostartDir() / "lanacoin.desktop";
}
...
HelpMessageBox::HelpMessageBox(QWidget *parent) :
    QMessageBox(parent)
{
    header = tr("Lanacoin-Qt") + " " + tr("version") + " " +
        QString::fromStdString(FormatFullVersion()) + "\n\n" +
        tr("Usage:") + "\n" +
        "  lanacoin-qt [" + tr("command-line options") + "]                     " + "\n";
